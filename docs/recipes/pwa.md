...
area: frontend
canonical: [apps/web-app/vite.config.mts, apps/web-app/ngsw-config.json, libs/shared/src/lib/state/sw-update.store.ts, apps/web-app-e2e/src/pwa-update.e2e.spec.ts]
updated: 2026-08-16
---
...
- `apps/web-app/vite.config.mts` — `VitePWA` plugin (manifest, `ngsw-config.json`); MF remotes stub `virtual:pwa-register` (`virtual-pwa-register-stub` plugin) since they don't run the SW
