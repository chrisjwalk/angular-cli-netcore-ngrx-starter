...
- **Recipe `Related` links are bare `<name>.md`** (correct when browsing on GitHub) — a marked extension in `vite.config.mts` (`content.markedOptions.extensions`) rewrites them to `/recipes/<name>` at render time; SWA config 404s stray `/*.md` URLs so they don't bounce to the home page via the SPA fallback.
