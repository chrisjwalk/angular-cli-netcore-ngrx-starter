...
test.describe('PWA update notification', () => {
  ...
  test('should register a service worker', async ({ page }) => {
    // PWA is disabled in dev mode (devOptions.enabled: false).
    // This test only works against a production build where the SW is
    // generated by vite-plugin-pwa and served from the build output.
    test.skip(
      !process.env['BASE_URL'],
      'PWA disabled in dev mode; run against production build to verify',
    );
  ...
  test('should show update notification when SW detects new version', async ({
    ...
    // PWA is disabled in dev mode (devOptions.enabled: false).
    // This test requires a production build + route interception.
    test.skip(
      !process.env['BASE_URL'],
      'PWA disabled in dev mode; run against production build to verify',
    );
